package pe.edu.cibertec.seguridad.exception;
...
import pe.edu.cibertec.seguridad.exception.ResourceNotFoundException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> handleNoResourceFoundException(
        ...
        // Solo imprimimos un aviso amarillo, no un error rojo gigante
        log.warn("Recurso web no encontrado: {}", request.getRequestURI());

        ApiResponse.ErrorDetails errorDetails = ApiResponse.ErrorDetails.builder()
                .code("RESOURCE_NOT_FOUND")
                .details("El archivo HTML o recurso solicitado no existe en este microservicio.")
                .path(request.getRequestURI())
                .build();

        ApiResponse<Object> response = ApiResponse.error("Página no encontrada", errorDetails);
        return new ResponseEntity<>(response, HttpStatus.NOT_FOUND);
    }
    /**
